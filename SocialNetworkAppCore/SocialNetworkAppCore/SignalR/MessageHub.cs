...
namespace SocialNetworkAppCore.SignalR
{
    public class MessageHub : Hub
    {
        ...
        public async Task SeenMessage(string recipientUsername)
        {
            var userName = Context.User.GetUsername();
            var messages = await _unitOfWork.MessageRepository.GetMessageThreadDescending(userName, recipientUsername);            
            var groupName = GetGroupName(userName, recipientUsername);

            await Clients.Group(groupName).SendAsync("SeenMessageReceived", messages.FirstOrDefault().Id);
        }
